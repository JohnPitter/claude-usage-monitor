...
function main() {
  ...
  if (usage.five_hour) {
    ...
    parts.push(`${BOLD}5h${RESET} ${bar} ${color}${pct}%${RESET}${DIM}(${reset})${RESET}`);
  }
  ...
  if (usage.seven_day) {
    ...
    parts.push(`${BOLD}7d${RESET} ${bar} ${color}${pct}%${RESET}${DIM}(${reset})${RESET}`);
  }
  ...
  if (extra?.is_enabled) {
    ...
    parts.push(`${CYAN}$${used.toFixed(0)}/${limit.toFixed(0)}${RESET}`);
  }

  // Session cost from stdin
  const cost = session?.cost?.total_cost_usd;
  if (cost != null && Number(cost) > 0) {
    parts.push(`${DIM}session:${RESET}${CYAN}$${Number(cost).toFixed(2)}${RESET}`);
  }
