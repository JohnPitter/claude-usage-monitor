 ...
 * Checks recent assistant messages for "thinking" content blocks.
 * Tool-use-only messages are skipped since they never contain thinking
 * blocks even when thinking mode is active.
 */
function detectThinkingFromTranscript(transcriptPath) {
  ...
  try {
    ...
    // Walk backwards to find a recent assistant message with text content
    // (skip tool_use-only messages since they never have thinking blocks)
    for (let i = lines.length - 1; i >= 0; i--) {
      try {
        ...
        if (entry.type === "assistant" && entry.message?.content) {
          ...
          if (!hasText) continue; // skip tool_use-only messages
